            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-light text-dark">
                    ...
                    @foreach($posts as $post)
                        <tr>
                            <td style="width: 60%"><a
                                        href="{{route('user.post.edit',['slug' => $post->slug])}}">{{$post->title}}</a>
                            </td>
                            ...
                            <td style="width: 40%">
                                <a href="{{route('user.post.show',['slug' => $post->slug])}}" class="btn btn-primary"><i
                                            class="fa fa-info-circle"></i>
                                    View</a>
                                <a href="{{route('user.post.edit',['slug' => $post->slug])}}" class="btn btn-info"><i
                                            class="fa fa-pen"></i>
                                    Edit</a>
                                <button class="btn btn-danger btn-delete-post" data-post-id="{{$post->slug}}"
